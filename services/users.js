  ...
  try {
    return new Promise(async (resolve, reject) => {
      ...
      if (userRecord && userRecord.length) {
        ...
        if(reqBody.password ===  userRecord[0].password ){
          passwordIsValid =true
        } //  no  decryption  password used bco'z from frontend or postman need to  encryption 


        // let passwordIsValid = bcrypt.compareSync(
        //   reqBody.password,
        //   userRecord[0].password
        // );   // if wanna use   decryption  password just uncomment this line of code and comment the above line of code
  ...
  try {
    return new Promise(async (resolve, reject) => {
      ...
      if (userExist && userExist.length) {
        ...
        newUser.password = reqBody.password;  // no  decryption  password used bco'z from frontend or postman need to  encryption 
        // newUser.password = await bcrypt.hashSync(reqBody.password, 8);   // if wanna use   decryption  password just uncomment this line of code and comment the above line of code
